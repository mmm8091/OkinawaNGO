...
- Every new or reopened research work package must include an `## 意外发现登记` README section and `unexpected_findings_register_v1.csv`. Package-local `lead_only` observations do not enter conclusions, central facts, human-review queues, publication snapshots or frontend views.
...
- Bounded reconnaissance may follow a registered unexpected finding for at most three steps and at most ten observations per work package. Record the chain and stop reason; continuing beyond that boundary requires a separate question and principal decision.
...
- U.S.-presence multilayer network wave 1: `docs/us_presence_network_research_wave_v1.md`, `docs/us_presence_network_architecture_v1.md`, `docs/us_presence_network_wave1_principal_checkpoint_v1.md`, and `outputs/us_presence_*`
  ...
  - The four USN review packets and five-item principal architecture checkpoint are principal-confirmed outside the old central ledger. Their research-return merge is commit `4dccaf1`; the design receipt is frozen at `6e7bd51`. The principal has since authorized plan mode and temporary-copy tests only. `docs/us_presence_integration_plan_checkpoint_v1.md` and `outputs/us_presence_integration_plan_v1/` now project 37 actions, 18 deltas, 57 source clusters, the exact 43-row relation overlay and 33 LEG field-cell changes; 14 regression tests and the package validator report `PASS_SYNTHETIC_SANDBOX_ONLY` / `PASS_PLAN_PACKAGE_BLOCKED`. The real plan remains blocked by nine official-IRS receipt checks, eight derived source requirements, three unapproved selection frames, a sponsor-snapshot schema seam and separate central-apply authority. No central writeback, legacy-crosswalk expansion, publication adapter or frontend release is authorized. Treat the old `PASS_DESIGN_ONLY` manifest as a commit-bound historical receipt rather than silently rehashing mutable control documents.
- U.S.-presence multilayer network wave 2: `docs/us_presence_network_wave2_research_command_v1.md`, `docs/us_presence_network_wave2_w2_00_principal_checkpoint_v1.md`, and `outputs/us_presence_network_wave2_w2_00_v1/`
  - The principal approved the research command and all seven W2-00 checkpoint decisions on 2026-08-22. W2-00 freezes 9 selection frames, 152 anchors, 43 source receipts, 41 local artifacts, 24 change notes and 4 case scales. Three OESC→AWWA Schedule I rows are `human_checked`; all three official XMLs report recipient EIN 980227149.
  - W2-A/B/C/D/E are complete as `research_only` packages. AWWA is supported as a repeated intermediary but not an earmarked chain; the USO national and Okinawa layers remain separated by a missing regional-allocation layer; the 13 accountability episodes are heterogeneous and the six extra rows are gate/control rather than a true matched non-entry arm; 36/54 S0×A0 direct pairs reach a bounded public-record zero while people/recipient/funder layers remain incomplete; the historical package shows different record regimes plus a care-to-rights permeability case.
  - The principal explicitly blocked W2-F on 2026-08-22. The unqualified 47k figure label is corrected. The principal accepted `arm_not_established`, retained the 2004 Henoko pollution mediation as post-entry gate-control and registered T2-H; HR-USN2-04/05/06b bound the three recipient transactions, local relationship frames and MTS→Lions intermediary edge, with T2-I for the downstream originals. HR-USN2-01 is now complete: Brooke, Amber, Trinicia and Lesilee are principal-confirmed same-person resolutions, while Jen/Jennifer remains unresolved. The research-only confirmed overlay has three cross-organization service-side people over two actor pairs; AWWA–OESC is the only pair that also has human-reviewed money observations (three OESC→AWWA tax-period flows totaling USD39,158). This batch does not test service-to-accountability person coverage. The public-rule portion of the governance lead is now a formal research-only package with 25 official-rule facts, 9 administrative grammars and 10 comparison units; it supports private status plus installation authorization/monitoring/sanctions and cost-recovery terms, not affiliation, funding or coordinated control. Actor-specific command files remain lead-only and are not an expected deliverable. On 2026-08-23 the principal promoted the DoD common institutional core to the first report finding, promoted the AWWA person-money-governance mechanism to the service-side core, and fixed legitimacy at the existing weak conclusion. DoD remains a typed `system_interface`, not an NGO bridge; generic installation rules do not create a command→AWWA edge. The W2-D USO-award citation was corrected and the principal overlay approves interpretive position only, with no fact-status upgrade. The information-disclosure pilot corrects U.S. fee-category semantics and narrows the three unsent drafts to single-period, single-custodian micro-requests; no request was sent. Recipient-dedup and role-compartmentalization observations remain lead-only. `docs/us_presence_network_wave2_detective_checkpoint_v3.md` is the current interpretive pause. W2-F still awaits HR-USN2-02/03, 06a/06c, 07–12, 13.2–13.4 and 14 plus actual registration for the three disclosure routes. W2-G central writeback, publication adapter and frontend release remain separately unauthorized.
...
- Live human-review ledger: `docs/principal_human_review_remaining_v19.md`
  - No pre-USN central online human-decision field is blank. HR-035 Batch 2's 23 decisions are principal-confirmed and centrally merged through the dedicated idempotent merger. Twelve local/new-primary-material items remain; AI157 and AI158 are completed `defer_second_source` decisions and now form online enrichment leads rather than blank human-review items. The USN packets and checkpoints are tracked outside the old central ledger. They remain unmerged into central facts and have no central writeback authority. W2-A/B/C/D/E research-only work is complete; HR-USN2 and information-disclosure actions remain outside the old central ledger, and central apply remains a separate future decision.
