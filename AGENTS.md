...
- Exploration system: `outputs/exploration_system_data_v1/`, `prototypes/nr3_explorer/`, and `docs/nr3_handoff_v1.md`
  - The last materialized frontend snapshot hides A072 and AI068 from default use and separates reviewed/candidate dyadic, administrative, aggregate, event and case-role layers. After HR-029, the adapter builds the current 125-reviewed/158-research actor-issue layer in a temporary output and its 22 contract tests pass; current R10 renderer tests (5) and R1/R2 gate tests (7) also pass. Do not claim the full 124-test historical suite is green: 19 failures and 5 errors remain in pre-freeze count fixtures, H1/H2/H3 snapshots, the pre-human HR-034 builder and the HR-018-era report-SHA reconciler. Regenerate the frontend only after preserving the current uncommitted UI work.
