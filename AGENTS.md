...
- Live human-review ledger: `docs/principal_human_review_remaining_v19.md`
  - No pre-USN central online human-decision field is blank. HR-035 Batch 2's 23 decisions are principal-confirmed and centrally merged through the dedicated idempotent merger. Twelve local/new-primary-material items remain; AI157 and AI158 are completed `defer_second_source` decisions and now form online enrichment leads rather than blank human-review items. The USN packets and five-item checkpoint are principal-confirmed and tracked outside the old central ledger. They remain unmerged into central facts and have no central writeback authority. Plan/sandbox construction is complete; the next safe work is source-receipt freezing and the explicit selection-frame/schema decisions listed in `docs/us_presence_integration_plan_checkpoint_v1.md`, not central apply.
