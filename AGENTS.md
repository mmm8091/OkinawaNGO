...
- U.S.-presence multilayer network wave 1: `docs/us_presence_network_research_wave_v1.md`, `docs/us_presence_network_architecture_v1.md`, `docs/us_presence_network_wave1_principal_checkpoint_v1.md`, and `outputs/us_presence_*`
  ...
  - The four USN review packets and five-item principal architecture checkpoint are principal-confirmed outside the old central ledger. The approved next action is a controlled integration design with expected field-level diff and idempotent tests; central writeback, publication adapter and frontend release remain unauthorized. The pre-return machine contracts still use L1/L2/L3 suffixes as legacy aliases for approved LEG1/LEG2/LEG3 semantics; migrate them mechanically before integration.
