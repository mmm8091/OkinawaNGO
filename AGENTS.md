...
- U.S.-presence multilayer network wave 2: `docs/us_presence_network_wave2_research_command_v1.md`, `docs/us_presence_network_wave2_w2_00_principal_checkpoint_v1.md`, and `outputs/us_presence_network_wave2_w2_00_v1/`
  ...
  - The principal explicitly blocked W2-F on 2026-08-22. The unqualified 47k figure label is corrected, and three information-disclosure drafts are frozen as `draft_not_sent`. The principal accepted the bounded non-entry result `arm_not_established`, retained the 2004 Henoko pollution mediation as post-entry gate-control and registered the 2018 Ryukyu Cement receipt/refusal original as named task T2-H. HR-USN2-04/05/06b are principal-confirmed: the three recipient transactions remain bounded, Kana-san and Ambitious support locally authored relationship frames without narrative uptake or LEG3, and MTS→Lions stops at the intermediary while T2-I seeks the downstream originals. The AWWA next-filing check found no 2025-05-31 return in the public IRS index as of 2026-08-22, so the Himawari records remain separate without inferring non-filing. Person supplements strengthen three service-side same-person candidates but do not test cross-ecology person coverage. Three bounded detective packages add only `research_only` structure: installation PO rules suggest private liability plus command authorization/monitoring and cost recovery rather than MCCS affiliation or proven subsidy; conditional scenarios make AWWA–OESC the sole person/resource overlap only if HR-USN2-01c is accepted; a `lead_only` 137-row audit over 11 local endpoints registers no closed independent multi-provider convergence after AWWA-chain, near-name and same-event deduplication, but this is not a current finding or publishable zero. `docs/us_presence_network_wave2_detective_checkpoint_v1.md` is the current interpretive pause. W2-F still awaits the remaining HR-USN2 decisions and actual send/registration of the three disclosure requests. W2-G central writeback, publication adapter and frontend release remain separately unauthorized.
