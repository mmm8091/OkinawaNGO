...
- Actor-issue table: 294 historical rows. The current analytical layer has 283 active edges, 125 human-reviewed and 158 candidate; 116 active actors are connected and 5 are isolated. Eleven rejected/deactivated/excluded rows remain history-only. AI068 and rejected AI178 are excluded from the default Okinawa narrative.
...
- Exploration system: `outputs/exploration_system_data_v1/`, `prototypes/nr3_explorer/`, and `docs/nr3_handoff_v1.md`
  - The frontend is regenerated from the merged post-freeze layer: 125 reviewed/158 research actor-issue edges, with 67 frozen-bounded, 58 accepted-unfrozen, 44 scope-reviewed/fact-pending and 114 fact-pending states. It hides A072 and AI068 from default use and separates dyadic, administrative, aggregate, event and case-role layers. Five routes pass in-app browser checks at 1280×900 and 390×844; mobile document width is 375px and the app console is clean. Five reviewed central lifecycle records exist (four from the latest LCR decisions), but `genealogy_anchors` still exports 0 and L2 remains an explicit frontend gap. The 22 adapter tests, 5 current R10 renderer tests and 7 R1/R2 gate tests pass. Do not claim the full 124-test historical suite is green: 19 failures and 5 errors remain in pre-freeze count fixtures, H1/H2/H3 snapshots, the pre-human HR-034 builder and the HR-018-era report-SHA reconciler.
...
- Live human-review ledger: `docs/principal_human_review_remaining_v18.md`
  - HR-035 Batch 2 is formally dispatched with 23 online decisions: 18 E4 actor–issue facts whose HR-019 scope is already reviewed, plus five one-per-actor identity companions. Twelve local/new-primary-material items also remain: HR-017 nine, HR-018 two already deferred pending new material, and HR-024/A073 one. Current formal open total: 35.
- First post-audit candidate packages and controlled merge:
  ...
  - `outputs/R01_R02_actor_issue_v1/`: history audit covers 122 registry rows and 294 actor-issue rows; current figures use 121 active actors and 283 active edges (125 reviewed/158 candidate), with 116 connected and 5 isolated. HR-019, HR-035 Batch 1 and HR-010 batch 6 are merged; eleven invalidated rows remain history-only.
...
- Second post-audit online wave:
  ...
  - `outputs/human_review_orchestration_v1/`: historical pre-merge orchestration snapshot; use `docs/principal_human_review_remaining_v18.md` for the live 23-online-plus-12-local ledger.
  - `outputs/R03_strict_place_issue_v1/`: 305 active same-source actor-place-issue triples; 298 are E3/E4, 71 dual-human-reviewed, and 97 have formal event attachment. The retired Cartesian projection is only a sensitivity upper bound.
...
- If delivered today, the conservative client verdict remains reject-and-rectify: the 120 minimum, first same-source place-issue layer and dispatched online HR freeze are repaired, but 158 active actor-issue edges remain candidates and all final DOCX/PDF, paper, PPTX, Sakishima dossier, public-data bundle and final codebook/lint remain incomplete.
...
- Safe current message: all returned online reviews through HR-035 Batch 1 are integrated; HR-035 Batch 2 is now formally dispatched with 18 actor–issue fact decisions plus five identity companions. The registry has 122 history rows/121 active actors and actor–issue has 294 history/283 active edges (125 reviewed/158 candidate). The place-issue replacement has 305 active same-source triples (71 dual-reviewed). Twelve local/new-primary-material items remain in addition to the 23 online decisions. The exploration frontend is regenerated and browser-checked; its remaining structural gap is the zero-anchor L2 export despite five reviewed central lifecycle records. Final contract artifacts still do not exist.
