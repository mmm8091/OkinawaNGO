...
前述第 1–11 节保留为第一次/第二次同步时的历史快照，不再用于判断当前内部进度。2026-07-13 当前数据为 118 个组织级 actors、159 sources、218 actor–issue 候选/人审边、124 actor–place 候选边、43 funding/support/non-funding relation 样本边；另有 49 条正式 evidence note、64 条正式 actor–event–venue、6 案 R8 case registry 和 27 条人审角色。HR-011/012/014/015 已回写；HR-013 未收到结论。A087–A101 仍待复核。
...
Registry 必须满足原方案 120–180 的一期范围；HR-015 撤出九个仅有 E2 一次署名线索的名称后，当前 118，重新低于数量下限。扩样仍按模块缺层推进，至少补 2 个组织身份／持续性可核的 actor；不得把旧 A077–A085 重新入表凑数。Tier B 12 个组织只有在形成独立本土声援层并服务 R5/R6 时才分层纳入；Tier C 继续事件限定。

下一轮对外沟通前，应优先形成：

1. 使用 HR-011/012/014/015 正式表重生基础图；HR-013 继续明确空缺，不代填。
2. 从 corrected QA 层收口 R4 先岛 framing、R9 公投程序和 R10 行政协作；原始候选包不直接并主表。
3. 补足 registry 120 下限并继续 A087–A101 分类／关系复核，再逐模块对照 R1–R11 done_when。
4. 完成方案指定五图、各模块解释性成果、25–35 页报告、8k–12k 论文和 15–20 页 PPT；由精确证据缺口反推当地协作者任务包。
