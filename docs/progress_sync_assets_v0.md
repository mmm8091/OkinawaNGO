...
Registry 必须满足原方案 120–180 的一期范围；当前 123 已达到数量下限，但不代表模块饱和或一期验收。扩样仍按模块缺层推进，必要时可超过 180。2020 MMC Tier A 9 个组织已入表；Tier B 12 个组织只有在形成独立本土声援层并服务 R5/R6 时才分层纳入；Tier C 31 个一次性署名者维持事件限定。

下一轮对外沟通前，应优先形成：

1. 已完成严格方案验收审计；R1–R11 均有当前解释与 done_when。
2. 继续补基础缺表、registry、R7/R8/R9/R10 和先岛专题等明显缺口。
3. 完成方案指定五图、各模块解释性成果、25–35 页报告、8k–12k 论文和 15–20 页 PPT。
4. 从模块和报告的精确证据缺口反推当地协作者正式任务包。
