...
前述第 1–11 节保留为第一次/第二次同步时的历史快照，不再用于判断当前内部进度。2026-07-13 当前数据为 118 个组织级 actors、198 sources、218 actor–issue 候选/人审边、124 actor–place 候选边、43 funding/support/non-funding relation 样本边；另有 49 条正式 evidence note、64 条正式 actor–event–venue、6 案 R8 case registry 和 27 条人审角色。HR-011/012/014/015 已回写；HR-013 未收到结论。A087–A101 仍待复核。新增来源 S160–S198 中 37 条已归档、2 条为 SSL 失败，全部保持 `ai_seeded`。
...
Registry 必须满足原方案 120–180 的一期范围；HR-015 撤出九个仅有 E2 一次署名线索的名称后，当前 118，重新低于数量下限。R1/R2 审计又显示 18 个在表 actor 尚无 edge-level evidence，9 个扩样候选全部 `count_ready=no`；因此先补活已有样本，再从能修复模块缺层且闭合持续身份／一期直接连接的候选中至少补 2 个。不得把旧 A077–A085 重新入表凑数；Tier B 只有形成独立本土声援解释层时才分层纳入，Tier C 继续事件限定。

下一轮对外沟通前，应优先形成：

1. 使用 HR-011/012/014/015 正式表重生基础图；HR-013 继续明确空缺，不代填。
2. R4／R9／R10 已从 corrected QA 层生成正式数据、brief 和六张图；R6/R7/R11 又形成 69 条正式事实、三张路径／场域／进入方式图。HR-016／017／018／021 保留未决判断，原始候选包不直接并主表。
3. R1/R2 已完成 118×26 全量议题网络与分类／共现／bridge 四图，R5 已完成三次名单 169 条观察与两图，六维 coverage 图已完成；HR-019／020 控制未决分类和身份，coverage 不另设 HR-023。
4. 下一轮优先补 18 个议题孤立 actor、完成 9 个扩样候选的直接连接检索、补 R8 多案例比较图，并把 25–35 页报告、8k–12k 论文和 15–20 页 PPT 推进为正式交付；由精确证据缺口反推当地协作者任务包。
