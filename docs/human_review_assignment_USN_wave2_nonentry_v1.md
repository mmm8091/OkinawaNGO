...
Status: **principal decision pending**
...
|---|---|---|---|---|
| HR-USN2-NE-001 | Accept the frozen true-nonentry gate and the bounded result `arm_not_established`? | `accept` / `revise` / `reject` |  |  |
| HR-USN2-NE-002 | Retain the 2004 Henoko pollution-mediation case only as a post-entry/pre-substantive gate-control comparator? | `accept` / `revise` / `reject` |  |  |
| HR-USN2-NE-003 | Does this bounded failure complete the W2-F prerequisite, or should a new-primary/local retrieval task continue? | `accept_bounded_failure` / `continue_primary_retrieval` |  |  |
