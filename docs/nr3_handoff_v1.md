...
| 演示层只能搜 23 个 actor，17 个 edge-isolated 不可搜 | 搜索改为全量 122 actor（名称／别名／ID 匹配）下拉选择，A073 实测可达；画布筛选保留原逻辑 |
...
- 组织（V2）：59 条已审关联默认层；全量 122 actor 搜索（含别名与 ID）；类型／议题筛选；节点详情（议题、地点、事件记录、身份来源）；画布缩放平移。
- 时间（第五页）：一期方案四个时段节点、12 个已核事件按年组织、参与者跳组织页、组织谱系诚实缺口（0 锚点）。
- 路径（V3）：9 个已核 episode 六阶段阶梯，状态直读数据；episode 六阶段对比；参与组织、来源、关联案件。
- 证据（V4）：六维度 125 单元，facet 条形与来源类型×归档矩阵，机制解释面板。
- 研究视图：演示／研究全局开关，182 待审议题边（虚线）＋263 待审三元组＋119 待审地点边＋4 待审 episode＋4 分析种子，全部带待审标记；措辞经专项修正。
...
- 数据隔离：122 actors；9 demo episode；TE10–TE13 仅在 research；A094 未进入；S051 保持 E0、不可支持主张。
- 探索链 A（地图）：地图→地点（选区）→episode（面板入口）→来源（路径页芯片→抽屉）实测通过。
- 探索链 B（组织）：A073 实测证明全量 122 registry actor 搜索可达（该 actor 当前 0 议题／
  0 事件，不承担事件链）；A002 实测证明组织→议题→事件记录→时间页 2003 链通过。
...
- 演示层≠全量数据：122 个组织中仅 23 个有已审议题边；演示层是"可公开辩护"的子集。
