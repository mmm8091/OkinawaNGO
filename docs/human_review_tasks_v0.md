...
| 任务 | 状态 | 当前结论 |
|---|---|---|
| HR-010 | completed / merged | A102-A106 已完成；A094 已按范围修正撤出；batch 6 接受46条并新增 AI249–AI294，HR010-B6-019 保持 defer。 |
...
| HR-034 | completed / merged | 45 条 source-log 行、AI068 及4条表级政策已按字段语义迁移；legacy 状态没有被批量冒充为 `human_checked`。 |

主 registry 保留 **122 条历史记录**，其中 A072 是合并到 A071 的重复组织 tombstone，当前为 **121 个有效 actor**，仍达到原方案 120–180 下限。当前中央表另有 **295 sources、294 条 actor–issue 历史边／283 条有效边、135 条 actor–place 历史边、67 条 AEV**。来源归档状态为 273 archived、2 manual_archived、18 failed、2 non-URL；归档失败不否定来源内容。
...
### HR-010 新增 E4 主体的分类与入网复核 — completed / merged

状态（2026-07-20）：A102–A106 已完成；A094 已撤出；A087–A093、A095–A101 的 batch 6
已由负责人完成47项边级决定（46 accept／1 defer）。以下保留原任务要求。
...
当前状态（2026-07-20）：HR-024 的线上决定已合并；A073 维持 `online_exhausted`／E0，
作为1项当地材料决定继续开放。A087–A093、A095–A101 的 HR-010 批6共47条已由负责人
决定：46条接受并落为 AI249–AI294，HR010-B6-019 暂缓且未生成中央边。以下保留原任务
说明作为审计轨迹。
...
### HR-029 schema 与 alias 冻结复核 — completed / merged

状态（2026-07-20）：HR-010 batch 6、LCR001–004 与 HR-034 合并后已最终重生505个统一候选，41条决定全部由负责人确认并完成中央冻结。P004/P010 保持地域／实体分层，18个 `R10_VENUE` 占位已全部处置。
...
### HR-031 报告解释强度复核 — completed / applied
...
### HR-035 actor–issue 事实与字段冻结 — Batch 1 completed / merged

状态（2026-07-20）：负责人已确认全部 15 条决定：6 `accept`、8 `revise`、1 `reject`。
AI106 改用 S128/S129；AI164 按完整事件期名称和 E3 谱系边界冻结；AI178 因错误政治 polarity
被拒绝并要求调整 HR-019 scope 的数据落点；AI231 当前只冻结到 2022 请愿。中央表及下游
R1/R2、strict place–issue 与 coverage 包均已受控合并／重生。本任务只决定现有来源是否足以
支持确切 actor—issue 映射，不重做 bridge。
...
1. HR-010 批6的47条 actor–issue 边级证据；
2. LCR001–004 的4条生命周期个案；
3. HR-034 的50条 legacy status crosswalk／表级政策；
