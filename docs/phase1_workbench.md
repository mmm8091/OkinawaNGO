...
22. 对美主线侦察补包继续保持 `research_only`。除 Himawari／Lions／人物／Ambitious 补查外，新完成三项结构侦察：基地 PO 规则指向“私人责任＋command 准入／监管＋成本回收支持”，不是 MCCS 隶属或已证资金支持；人物情景显示，若 01c Amber 获接受，AWWA—OESC 才形成目前唯一的人物候选＋已核资源流重叠；另一个 `lead_only` 包对6表137行／11地方端点做去重，在该包抽取框内尚无闭合的独立多 provider 汇聚，但此观察不进入当前结论或发布零值。解释检查点为 `docs/us_presence_network_wave2_detective_checkpoint_v1.md`；均未进入中央、HR 决定、publication 或前端。
