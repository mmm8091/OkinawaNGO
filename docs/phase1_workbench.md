...
22. 负责人已完成 HR-USN2-01：Brooke、Amber、Trinicia、Lesilee 合并，Jen／Jennifer 保持 unresolved。负责人确认 overlay 形成 AWWA—KOSC 两名共享人物、AWWA—OESC 一名共享人物；后者另有三期 OESC→AWWA 已核资金流，累计 USD39,158，但人物与资金继续分层，且本批只覆盖服务侧。基地治理公开规则层也已从 lead 升为正式 `research_only` 包：两份规则、25 条规则事实、9 类行政语法、10 个比较单元，支持“私人主体＋基地运作许可”，不支持 funding／affiliation／control；未取得的章程、季度账、银行流水仍留 `lead_only`，不写成交付预期。信息公开路径另形成三个未发送微请求，先做单年度、单保有层级试验。当前解释检查点已更新为 `docs/us_presence_network_wave2_detective_checkpoint_v3.md`；中央、publication、前端与 W2-F 均未改变。
23. 负责人于 2026-08-23 固定三项解释位置：DoD 共同制度核心升为报告第一结构性发现，但作为 `system_interface` 与 NGO bridge 分开；AWWA 升为“嵌入基地治理制度的人物—资金复合网络”这一服务侧核心机制，通用规则不得画成 command→AWWA 直接边；合法性维持弱结论，不再作为 W2-F 开放判断。当前检查点为 `docs/us_presence_network_wave2_detective_checkpoint_v3.md`。W2-D 的 USO award 锚点误引已修正，负责人 overlay 仅批准解释位置；W2-F、W2-G、中央、publication 与前端门禁不变。
