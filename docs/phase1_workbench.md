...
18. USN 第一轮 research-only 包、四份正式人工任务及五项研究架构决定均已完成；权威回传已以 `4dccaf1` 合入主线程。受控集成设计、37项预期动作、18项表变化、35条测试和 Kimi 探索证据审计现见 `docs/us_presence_controlled_integration_design_v1.md`、`outputs/us_presence_controlled_integration_design_v1/` 与 `docs/kimi_exploration_evidence_triage_v1.md`，validator 为 `PASS_DESIGN_ONLY`。下一检查点是负责人是否批准只生成 merger 的 plan／临时副本测试；中央事实层、43行 crosswalk、publication adapter 和前端仍未写入。
