...
目的：把当前 123 个 actor、143 条 source 和人工复核状态转成可沟通、可继续探索的图件。此包不是最终报告；它用于下一次沟通前验证叙事，并暴露下一轮补源方向。HR-010 批5已审 A102-A106；A087-A101 仍只完成 E4 身份级合并。
...
1. 手工处理 11 条自动归档失败来源；失败状态不等于证据不存在。
