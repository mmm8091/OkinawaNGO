...
def save_function_ecology(actors: list[dict[str, str]]) -> None:
    ...
    for actor in actors:
        if (
            "safe identity merge" in actor.get("notes", "")
            and actor.get("review_status") != "human_checked"
        ):
            family = "待人工分类"
        else:
...
def write_audit() -> None:
    rows = [
        ("R1", "组织分类与组织生态", "仅有早期 actor class 计数图", "缺功能层与来源层的交叉解释", "fig1_functional_ecology.png", "补图已出／模块未验收", "报告第3节"),
        ("R2", "组织—议题网络", "bridge network + shortlist", "只显示桥接 actor，非完整网络；E2 需保留边界", "保留现图；完整网络待报告按需", "现图可保留／模块未验收", "报告第4节"),
        ("R3", "地点与空间分布", "旧地点—议题宽投影（已退役）", "旧矩阵未要求地点与议题同源／同事件；正式 MA002 缺失", "fig2_actor_place_matrix.png 可作组织—地点补图；另建同源／同事件三元事实 MA002", "补图已出／正式核心图缺失", "报告第5节"),
        ("R4", "环保／生活安全与军事设施", "旧地点—议题宽投影（已退役）", "不能据旧投影判断场域差异；与那国还需避免环保化", "用 R4 文本证据和 MA002 严格三元事实重建", "旧图不得正式使用／模块未验收", "报告第5节"),
        ("R5", "共同行动网络", "样本构成 + event repertoire 时间线", "尚不足以证明稳定联盟；重复参与很少", "保留事件感知时间线，不补静态联盟图", "现图可保留／模块未验收", "报告第6节"),
        ("R6", "跨国／国际倡议", "Henoko internationalization pathway", "当前集中边野古／大浦湾，其他国际路径较薄", "保留现图；当地或后续源补足后再扩", "现图可保留／模块未验收", "报告第4/6节"),
        ("R7", "场域与对象转移", "路径图提供单案例", "缺跨事件、跨制度场域的可比时间数据", "暂不作图，先补 event/venue 字段", "数据不足", "二期／报告局限"),
        ("R8", "法律／政策／环境程序", "路径图 + lawsuit role table", "缺多案件／程序的统一事件表", "暂不重复作图；先扩法律事件", "现图可保留／模块未验收", "报告第6节"),
        ("R9", "选举／公投连接", "event repertoire 含公投", "只有发起组织，非完整参与网络", "保留时间线，避免把单节点当全体", "现图可保留／模块未验收", "报告第5/6节"),
        ("R10", "公开资源／行政协作", "support/funding 样本早期计数", "资金、委托、服务、非资金关系曾混在一起", "fig3_support_service_layers_strict.png", "补图已出／模块未验收", "报告第7节"),
        ("R11", "外来 NGO 与国际倡议", "Henoko pathway + event layer", "军属服务与倡议层需明确分开", "fig1 + fig3 共同呈现功能分层", "边界图已出／模块未验收", "报告第6/7节"),
        ("R12", "媒体可见度", "无", "一期已排除，且需要报刊数据库", "不作图", "扩展模块", "报告局限"),
        ("R13", "人物—组织互锁", "无", "人物字段与人工复核不足", "不作图", "扩展模块", "报告局限"),
        ("R14", "组织谱系与长期演变", "无", "扩展模块；需要旧报纸、档案和组织史资料", "一期不作最终图，仅保留谱系线索", "扩展模块", "报告局限"),
        ("B0", "覆盖／证据偏差", "evidence gap map + coverage brief", "现有图偏状态计数，未比较时间、地点、功能层和来源可见度", "升级为多维 coverage audit 后重画", "部分覆盖", "报告第8节"),
    ]
    write_csv(
...
2026-07-14 方法更正：现有地点—议题矩阵只保留为历史探索产物，不得用于正式结论；它需要由同源／同事件三元事实 MA002 取代。边野古国际化路径、桥接 actor 图与事件 repertoire 时间线也继续受相应人工／解释 gate 控制。不应为了数量再画一张静态“联盟网络”。
