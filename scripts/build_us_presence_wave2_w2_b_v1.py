...
def source_receipts() -> list[dict[str, object]]:
    ...
    specs = [
        ...
        ("W2B2-SR010", "United Service Organizations", "Here For Those Who Serve: How USO Okinawa Centers Are Uplifting Units Across The Island", "official_website", "https://pacific.uso.org/stories/1097", ART / "uso_pacific_story_center_service_examples.html", "text/html", "Published 2025-07-10; six center subsections", "W2B2-SC009;W2B2-SC010;W2B2-SC011;W2B2-SC012;W2B2-SC013;W2B2-SC014;W2B2-FB006", "local_frozen_research_only"),
        ("W2B2-SR011", "United Service Organizations", "USO Okinawa Furthers Support of Soldiers During Milestone Birthday", "official_website", "https://pacific.uso.org/stories/998", ART / "uso_pacific_okinawa_outreach_torii.html", "text/html", "Published 2025-06-13; six physical locations and Torii/White Beach outreach paragraphs", "W2B2-HS015;W2B2-HS016;W2B2-SC015", "local_frozen_research_only"),
        ...
        ("W2B2-SR034", "United Service Organizations", "2024 audited consolidated financial statements", "official_audited_financial_statement", "https://www.uso.org/document/513", reused / "uso_2024_financial_statement.pdf", "application/pdf", "p.8 functional expenses; total program services", "W2B2-WF009", "reused_w2_00_frozen"),
    ]
    rows = []
...
def hierarchy_rows() -> list[dict[str, object]]:
    ...
    add("W2B2-HS004", "2026-08-21", "current_directory_snapshot", "region", "USO Indo-Pacific", "area", "USO Okinawa", "okinawa_area", "Okinawa", "listed", 8, "typed_presences", "W2B2-SR002;W2B2-SR003", "Homepage locations; 2025 spotlight six-center paragraph", "USO Okinawa has six centers plus a terminal presence and an area office in the current directory.", "Do not call all eight entries homogeneous centers or legal entities.")
    centers = [
    ...
    for row_id, child, location in centers:
        add(row_id, "2026-08-21", "current_directory_and_2025_narrative", "area", "USO Okinawa", "site", child, "operating_center", location, "listed", 1, "center", "W2B2-SR002;W2B2-SR003", "Homepage locations and 2025 six-center list", "This is one of the six named operating centers.", "Do not assign a center-level budget, service count or separate法人格.")
...
def federal_rows() -> list[dict[str, object]]:
    ...
    conflict = "USO_AWARD_72M_VS_ACCOUNT_41_212M"
    ...
    add("W2B2-FA009", "federal_account_reporting", "funding_rollup", "total_transaction_obligated_amount", 41212463.29, "USD 41,212,463.29", "USD", "reporting records through FY2024 in retrieved endpoint", "one funding/awarding agency and one federal account", "W2B2-SR026;W2B2-SR029", "JSON root", "official_account_linked_rollup_semantic_conflict", conflict, "The account-linked rollup reports USD 41.212 million in transaction obligations for its reporting perimeter.", "Do not relabel this as the award total, cash paid or an amount available to Okinawa.")
    add("W2B2-FA010", "federal_account_reporting", "account_listing", "account_linked_obligation", 41212463.29, "USD 41,212,463.29", "USD", "retrieved current", "097-0100 Operation and Maintenance, Defense-Wide, Defense", "W2B2-SR027;W2B2-SR030", "$.results[0]", "official_account_linked_record", conflict, "The public account listing associates the rollup with federal account 097-0100.", "Do not infer a separate award or a geographic program allocation.")
    funding = [
...
def boundary_rows() -> list[dict[str, object]]:
    ...
    add("W2B2-FB005", "after_hours_nmcrs_financial_liaison", none, "No role observed.", "agent_interface", "Red Cross issues/helps with assistance after hours on NMCRS's behalf.", "principal_and_fund_source", "NMCRS identifies Red Cross as its after-hours liaison using NMCRS funds.", "Okinawa and global after-hours access", "confirmed_service_intermediation", "W2B2-SR022;W2B2-SR023;W2B2-SR024", "After-hours callout; corporate funding paragraph", "This is a formal service handoff: Red Cross interface, NMCRS program/funds.", "Do not code it as Red Cross funding NMCRS, a joint grant, organizational merger or political alliance.", "NMCRS also states it receives no Department of Defense funds.")
...
def waterfall_rows() -> list[dict[str, object]]:
    rows = [
        common({"stage_order": 1, "stage_id": "W2B2-WF001", "lane": "federal_award_chain", "stage_label": "DoD/WHS -> USO, Inc.", "organizational_level": "national_prime", "known_amount": 72000000, "value_text": "USD 72.000m award-level obligation", "currency": "USD", "period": "2023-09-30/2028-09-29", "measurement_type": "award_obligation", "visibility_status": "known_award_level", "source_receipt_ids": "W2B2-SR006", "reason_not_additive": "Award-level cumulative obligation; not cash expenditure or local allocation.", "next_gap": "No public award-to-region allocation.", "allowed_claim": "A national DoD/WHS prime award is visible."}),
        common({"stage_order": 2, "stage_id": "W2B2-WF002", "lane": "federal_award_chain", "stage_label": "Federal-account reporting", "organizational_level": "national_account_link", "known_amount": 41212463.29, "value_text": "USD 41.212m account-linked rollup", "currency": "USD", "period": "retrieved reporting records through FY2024", "measurement_type": "account_linked_transaction_obligation", "visibility_status": "known_semantic_conflict", "source_receipt_ids": "W2B2-SR025;W2B2-SR026;W2B2-SR027", "reason_not_additive": "Different reporting perimeter; includes a -USD 6.788m adjustment and omits the visible 2025 continuation in the inspected account records.", "next_gap": "Cannot reconcile to 72m as a cash-flow waterfall.", "allowed_claim": "The account layer exposes a reporting/perimeter conflict, not a second award."}),
        ...
        common({"stage_order": 5, "stage_id": "W2B2-WF005", "lane": "federal_award_chain", "stage_label": "USO Okinawa annual operating budget", "organizational_level": "okinawa_area", "known_amount": "", "value_text": "not publicly disclosed", "currency": "", "period": "current", "measurement_type": "local_budget", "visibility_status": "allocation_gap", "source_receipt_ids": "W2B2-SR002;W2B2-SR003", "reason_not_additive": "Local staff, volunteers, sites and service-population scale are non-money facts.", "next_gap": "Regional finance or center-level annual report required.", "allowed_claim": "USO Okinawa's service presence is visible while its annual budget is not."}),
        ...
        common({"stage_order": 8, "stage_id": "W2B2-WF008", "lane": "local_service_capacity", "stage_label": "USO Okinawa operating scale", "organizational_level": "okinawa_area", "known_amount": "", "value_text": "6 centers / 8 presences / 21 employees / 780+ volunteers / approx. 47k supported population", "currency": "", "period": "2025/current", "measurement_type": "nonmoney_capacity", "visibility_status": "known_nonmoney_facts", "source_receipt_ids": "W2B2-SR002;W2B2-SR003", "reason_not_additive": "Different units and definitions; none is a financial denominator.", "next_gap": "Same-definition local uses/visits/people and annual expenses missing.", "allowed_claim": "The local service apparatus has visible organizational capacity despite missing finances."}),
        common({"stage_order": 9, "stage_id": "W2B2-WF009", "lane": "national_financial_context", "stage_label": "USO 2024 consolidated program services", "organizational_level": "national_consolidated", "known_amount": 204912000, "value_text": "USD 204.912m", "currency": "USD", "period": "2024", "measurement_type": "consolidated_program_service_expense", "visibility_status": "known_separate_financial_perimeter", "source_receipt_ids": "W2B2-SR034", "reason_not_additive": "Organization-wide consolidated expense; not an award allocation and not an Okinawa budget.", "next_gap": "No regional expense schedule.", "allowed_claim": "This supplies national organizational scale only."}),
    ]
    return rows
...
def negative_rows() -> list[dict[str, object]]:
    base = [
        ...
        ("W2B2-NS002", "Does USO publish Okinawa service uses, visits and unique people in the same definitions as the global report?", "USO 2024 impact report and Okinawa official pages/stories", "Only an approximate supported-population statement and selected event counts were found; no same-definition local annual metrics.", "local_metric_definition_gap", "W2B2-SR001;W2B2-SR003;W2B2-SR010;W2B2-SR011", "Okinawa annual impact dashboard or area report", "Local capacity is visible but cannot be converted into a service-use allocation weight.", "Do not equate 47,000 supported population with annual users or uses."),
        ...
        ("W2B2-NS005", "Can the USD 41.212m account figure be made equivalent to the USD 72m award total?", "USAspending overview, transactions, funding rollup, accounts and periodic funding records", "The account rollup equals 24m - 6.78753671m + 24m; the third 2025 24m transaction is visible in history but not closed in inspected account rows.", "reporting_perimeter_or_timing_conflict", "W2B2-SR006;W2B2-SR007;W2B2-SR025;W2B2-SR026;W2B2-SR027", "later File C submissions or downloadable award/account records with availability periods", "The two official figures must remain separately typed.", "Do not add, substitute or present either as cash paid."),
        ("W2B2-NS006", "Is there a complete annual Okinawa center roster with opening/closing dates?", "2021 story, 2024 impact report, 2025 stories and current directory", "Site names and count vocabularies are available, but annual start/end dates are incomplete.", "site_history_incomplete", "W2B2-SR001;W2B2-SR002;W2B2-SR009;W2B2-SR011", "archived annual directories, leases, center opening/closure announcements", "The 7/6/8 difference can be typed but not turned into a complete lifecycle.", "Do not infer openings, closures or dissolutions from count changes alone."),
...
def change_rows() -> list[dict[str, object]]:
    rows = [
        ("W2B2-CN001", "site_count_semantics", "Treat 6 vs 8 as a count discrepancy.", "Official pages use center, location, presence, terminal, office and outreach with different dates.", "Use a dated typed hierarchy: 2021 seven listed locations; 2025 six centers; current eight typed presences.", "W2B2-HS001;W2B2-HS002;W2B2-HS004;W2B2-HS013;W2B2-HS014", "W2B2-SR002;W2B2-SR003;W2B2-SR009;W2B2-SR011", "No single site count is discarded; three vocabularies remain.", "Count changes no longer imply openings/closures.", "Confirm typed interpretation.", "ready_for_principal_review"),
        ("W2B2-CN002", "award_account_semantics", "Treat USD 41.212m as an unexplained duplicate/conflict field.", "The funding endpoints show it is the account-linked rollup of 24m - 6.7875m + 24m, while transaction history includes an additional 2025 24m.", "Preserve USD 72m award-level and USD 41.212m account-linked figures side by side; record reporting/timing conflict.", "W2B2-FA001;W2B2-FA004;W2B2-FA007;W2B2-FA008;W2B2-FA009;W2B2-FA011;W2B2-FA014;W2B2-FA015", "W2B2-SR006;W2B2-SR007;W2B2-SR025;W2B2-SR026;W2B2-SR027", "The 41.212 figure is now decomposed but not reconciled to 72m.", "It is not a second award or local share.", "Approve wording and whether later File C recheck is needed.", "ready_for_principal_review"),
        ...
        ("W2B2-CN004", "service_metric_types", "Use all USO counts as service volume.", "Uses, visits, people reached, supported population, volunteers and selected-event participants are different units.", "Keep every metric typed and prohibit conversion across definitions.", "W2B2-SC001;W2B2-SC002;W2B2-SC003;W2B2-SC005;W2B2-SC006;W2B2-SC007;W2B2-SC009;W2B2-SC010;W2B2-SC011;W2B2-SC012;W2B2-SC013;W2B2-SC014", "W2B2-SR001;W2B2-SR003;W2B2-SR010", "No local-use weighting is produced.", "Capacity can be described without a fabricated denominator.", "No additional decision unless a local annual metric appears.", "method_change_applied"),
        ("W2B2-CN005", "service_ecology_boundary", "Describe USO, Red Cross and NMCRS as parallel generic welfare services.", "Official pages reveal distinct cores and one specific Red Cross-NMCRS after-hours handoff using NMCRS funds.", "Build a function matrix and type the handoff as service intermediation, not funding/alliance.", "W2B2-FB001;W2B2-FB002;W2B2-FB003;W2B2-FB004;W2B2-FB005;W2B2-FB006;W2B2-FB007;W2B2-FB008;W2B2-FB009;W2B2-FB010", "W2B2-SR020;W2B2-SR021;W2B2-SR022;W2B2-SR023;W2B2-SR024", "One confirmed intra-service-ecology handoff is added to the research package.", "The three organizations are complementary/overlapping in typed functions, not interchangeable.", "Confirm relation semantics before any central proposal.", "ready_for_principal_review"),
        ("W2B2-CN006", "waterfall_design", "Draw a single numeric national-to-local waterfall.", "There is no region allocation, local observations mix dates/currencies, and national expense/award/account fields have different perimeters.", "Draw an allocation-visibility waterfall with separate national, local-input and non-money lanes.", "W2B2-WF001;W2B2-WF002;W2B2-WF003;W2B2-WF004;W2B2-WF005;W2B2-WF006;W2B2-WF007;W2B2-WF008;W2B2-WF009", "W2B2-SR006;W2B2-SR013;W2B2-SR016;W2B2-SR025;W2B2-SR026;W2B2-SR027;W2B2-SR034", "No synthetic Okinawa amount is generated.", "The missing allocation layer becomes the visual finding.", "Approve as W2-B closing figure.", "ready_for_principal_review"),
    ]
...
def principal_rows() -> list[dict[str, object]]:
    rows = [
        ("W2B2-PR001", "hierarchy_semantics", "Approve the dated 2021 seven-location / 2025 six-center / current eight-presence interpretation?", "accept_typed_dated_hierarchy", "The count difference could reflect unrecorded site lifecycle changes; the current evidence only resolves terminology, not every opening/closure.", "W2B2-HS001;W2B2-HS002;W2B2-HS004;W2B2-HS013;W2B2-HS014", "W2B2-SR002;W2B2-SR003;W2B2-SR009;W2B2-SR011", "Allows a site-definition timeline without false lifecycle claims."),
        ("W2B2-PR002", "federal_amount_semantics", "Approve USD 41.212m as an account-linked reporting figure that remains semantically distinct from the USD 72m award-level obligation?", "accept_parallel_typed_figures_and_conflict", "Later reporting may close the account layer to 72m; the current endpoint snapshot may be timing-lagged.", "W2B2-FA001;W2B2-FA009;W2B2-FA010;W2B2-FA011;W2B2-FA014;W2B2-FA015", "W2B2-SR006;W2B2-SR025;W2B2-SR026;W2B2-SR027", "Allows a defensible funding-chain gap statement."),
        ...
        ("W2B2-PR005", "service_function_relation", "Approve Red Cross as NMCRS's after-hours service intermediary using NMCRS funds?", "accept_service_intermediation_not_funding_or_alliance", "Operational practice may vary after the page snapshot; the source still states the current formal handoff.", "W2B2-FB003;W2B2-FB004;W2B2-FB005", "W2B2-SR022;W2B2-SR023;W2B2-SR024", "Adds one precise intra-service-ecology relation while preserving money provenance."),
        ("W2B2-PR006", "allocation_gap_claim", "Approve closing W2-B with 'national funding and local service presence are visible, but the regional monetary allocation layer is not public'?", "accept_bounded_layer_gap", "A grant agreement or regional annual report could later close the gap.", "W2B2-FA018;W2B2-WF001;W2B2-WF002;W2B2-WF003;W2B2-WF004;W2B2-WF005", "W2B2-SR001;W2B2-SR006;W2B2-SR025;W2B2-SR026;W2B2-SR027", "Provides the W2-B strongest conclusion without an Okinawa budget estimate."),
...
def w2d_endpoint_rows() -> list[dict[str, object]]:
    ...
    add(
        "W2B2-DE028", "service_intermediary_relation", "American Red Cross after-hours interface for NMCRS",
        "", "X008 American Red Cross Okinawa ↔ X009 Navy-Marine Corps Relief Society Okinawa", "",
        "after_hours_service_intermediation_using_nmcrs_funds", "", "2026-08-22", "current_page_snapshot",
        "W2B2-SR022;W2B2-SR023;W2B2-SR024", "NMCRS Okinawa and Get Help after-hours callouts",
        "official_function_relation", "service_interface_candidate", "Typed cross-organization service edge for W2-D",
        "Red Cross is an after-hours intake/issuance interface acting on NMCRS's behalf with NMCRS funds.",
...
def render_waterfall_svg(path: Path) -> None:
    """Render a visibility diagram; box sizes deliberately do not encode amount."""
    width, height = 1440, 900
    parts = [
        f'<svg xmlns="http://www.w3.org/2000/svg" width="{width}" height="{height}" viewBox="0 0 {width} {height}">',
        '<rect width="1440" height="900" fill="#fbfaf5"/>',
        svg_text(60, 62, "USO 全国—冲绳：公开记录在哪一层中断", 30, 700, "#0b3d42"),
        svg_text(60, 96, "金额框采用不同会计口径，不能相加；框宽不代表金额大小", 17, 400, "#617579"),
        svg_text(60, 146, "联邦拨款与组织财务", 18, 700, "#0d6b6e"),
        svg_card(60, 170, 290, 132, "DoD / WHS → USO, Inc.", ["USD 72.000m", "全国 prime award obligation", "2023-09-30—2028-09-29"], tone="teal"),
        '<path d="M350 236 H405" stroke="#0d6b6e" stroke-width="3" marker-end="url(#arrow)"/>',
        svg_card(410, 170, 310, 132, "联邦账户报送层", ["USD 41.212m", "24m − 6.788m + 24m", "与 72m 尚未闭合"], tone="amber"),
        '<path d="M720 236 H785" stroke="#ca8c25" stroke-width="3" stroke-dasharray="8 7" marker-end="url(#arrowAmber)"/>',
        svg_card(790, 170, 270, 132, "USO Indo-Pacific", ["组织层级可见", "地区金额未公开", "public allocation gap"], tone="gray", dashed=True),
        '<path d="M1060 236 H1115" stroke="#9ca8a4" stroke-width="3" stroke-dasharray="8 7" marker-end="url(#arrowGray)"/>',
        svg_card(1120, 170, 260, 132, "Japan / Okinawa", ["站点与人员可见", "年度预算未公开", "本地同口径 uses 缺失"], tone="gray", dashed=True),
        svg_text(60, 354, "同一组织的全国财务背景（单列）", 18, 700, "#35556d"),
        svg_card(60, 378, 430, 126, "USO 2024 consolidated", ["Program services: USD 204.912m", "组织合并费用，不是该 award 分配", "不能换算为冲绳预算"], tone="blue"),
        svg_text(540, 354, "冲绳层可见的独立事实", 18, 700, "#0d6b6e"),
        svg_card(540, 378, 390, 174, "具名本地资源流（个案）", ["AEC: USD 18k (2024)", "AK Kogyo: JPY 1m (2025)", "AEC: USD 16k (2025)", "OESC: USD 3.25k (2025)", "非穷尽、跨币种，不合计"], tone="teal"),
        svg_card(950, 378, 430, 174, "USO Okinawa 运作规模", ["6 centers / 8 typed presences", "21 employees / 780+ volunteers", "约 47k supported population", "这些不是金额或年度 uses", "站点、办公室、terminal 分开统计"], tone="teal"),
        svg_text(60, 614, "服务生态边界", 18, 700, "#0d6b6e"),
        svg_card(60, 638, 415, 152, "USO", ["中心、连接、士气与外展", "部分转衔与家庭项目", "本地服务量未形成年度同口径表"], tone="blue"),
        svg_card(512, 638, 415, 152, "American Red Cross", ["紧急通信、医院志愿、灾害支援", "在 NMCRS 非营业时段担任接口", "功能重叠不等于组织联盟"], tone="blue"),
        svg_card(964, 638, 416, 152, "NMCRS", ["无息贷款/补助、应急旅行", "预算教育与灾害救助", "Red Cross 接口使用 NMCRS funds"], tone="blue"),
        svg_text(60, 846, "最强可支持结论：全国资金与冲绳服务存在都可见，但连接二者的地区分配层没有公开闭合。", 20, 700, "#0b3d42"),
        svg_text(1380, 876, "W2-B · research_only · 2026-08-22", 13, 400, "#718184", "end"),
...
def validate_package(tables: dict[str, list[dict[str, object]]], receipts: list[dict[str, object]]) -> dict[str, object]:
    ...
    check("W2B-V019", handoff["relation_or_boundary"] == "confirmed_service_intermediation" and "NMCRS funds" in handoff["nmcrs_evidence"], "Red Cross/NMCRS handoff keeps service and money provenance typed")
    ...
    check("W2B-V023", len(interface_endpoints) == 1 and interface_endpoints[0]["role_or_relation_type"] == "after_hours_service_intermediation_using_nmcrs_funds", "W2-D exposes one precisely typed ARC/NMCRS service interface")
...
| `allocation_waterfall_v1.csv` | {counts['allocation_waterfall_v1.csv']} | 瀑布图使用的九层可见性数据 |
